...
function PlayerTask:ctor(player)
    ...
    self.bindRakssid = player.rakssid
    ...
    self.refreshTaskIds = {}
    self.timerKey = "UpdateTask:" .. tostring(self.bindRakssid)
...
function PlayerTask:onReadData(userId, data)
    ...
    if player ~= nil and string.len(data) == 0 then
        TaskManager:InitPlayerTaskInfo(player)
        return
    end
    local json_data = json.decode(data)
    self:OnReadMainTaskDataFromDb(player, json_data.mainTasks)
    self:OnReadDailyTaskDataFromDb(player, json_data.dailyTasks)
...
function PlayerTask:OnCreateTaskSuccess(taskId)
    if type(taskId) == "number" then
        ...
        if taskId == GameConfig.drawLuckyTaskId then
            HostApi.changeGuideArrowStatus(self.bindRakssid, GameConfig.drawLuckyPoint, true)
        end

        if taskId == GameConfig.shopTaskId then
            HostApi.changeGuideArrowStatus(self.bindRakssid, GameConfig.arrowPointToShopPos, true)
        end
    end
end
...
function PlayerTask:acceptMainTask(player, npc, taskId)
    ...
    taskData:CreateTaskContent(self.bindRakssid)
...
function PlayerTask:AddDailyTimer()
    ...
    if intervalTimr > 0 then
        local key = "taskRefresh:" .. tostring(self.bindRakssid)
...
function PlayerTask:OnCompleteTask(taskId, npcId)
    ...
    if taskId == GameConfig.drawLuckyTaskId then
        HostApi.changeGuideArrowStatus(self.bindRakssid, GameConfig.drawLuckyPoint, false)
    end

    local player = PlayerManager:getPlayerByRakssid(self.bindRakssid)

    if taskId == GameConfig.shopTaskId then
        HostApi.changeGuideArrowStatus(self.bindRakssid, GameConfig.arrowPointToShopPos, false)
        if player ~= nil then
    ...
    TaskHelper.dispatch(TaskType.CompleteNpcTaskNumEvent, self.bindRakssid, npc.id, 1)
end
...
function PlayerTask:RefreshTaskInfo(taskId, isRemove)
    ...
    if type(taskId) == "number" then
        ...
        if birdtask ~= nil then
            HostApi.sendBirdSimulatorTaskItem(self.bindRakssid, isRemove, birdtask)
        end
...
function PlayerTask:SaveData()
    ...
    DBManager:savePlayerData(self.userId, DbUtil.TAG_TASK, json.encode(data), true)
end
