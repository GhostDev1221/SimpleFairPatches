...
function GameBirdBag:initDataFromDB(data)
    ...
    self:initMaxCarryBirdNumFromDB(item.maxCarry)
    self:initMaxCapacityBirdNumFromDB(item.maxCapacity)
    self:initItemsFromDB(item.items)
end
...
function GameBirdBag:initMaxCarryBirdNumFromDB(maxCarry)
    ...
    TaskHelper.dispatch(TaskType.HaveBirdShellsEvent, player.rakssid)
end
    ...
    if moneyType == 3 then
        if player.money >= price then
            ...
            for i, v in pairs(FieldConfig.door) do
                if tonumber(v.type) == 0 and tonumber(v.entityId) ~= 0 then
                    if
                        tonumber(v.entityId) ~= 0 and
                            tonumber(player.userBirdBag.maxCarryBirdNum) >= tonumber(v.requirement)
                     then
