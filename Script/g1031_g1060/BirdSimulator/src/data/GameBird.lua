...
function GameBird:getValue(bird, name)
    ...
    local rate = BirdConfig:getRateByIdAndLevel(bird.birdId, bird.level)
...
function GameBird:selectPotralPositionForBirdById(birdId)
    ...
    --print("bird ", birdId, StringUtil.v2s(bird))
    if not bird then
        return Vector3.new()
    end
...
function GameBird:initDataFromDB(data)
    ...
    self:initDressFromDB(item.dress)
    self:initDiscoveredFromDB(item.discovered)
    self:initLastIndexFromDB(item.index)
    self:initBirdsFromDB(item.birds)
end
...
function GameBird:createBird(id, birdId, level, exp, status, partIds)
    ...
    if birdInfo ~= nil then
        ...
        TaskHelper.dispatch(TaskType.HaveLevelBirdEvent, player.rakssid)
    end
end
...
function GameBird:respawnBirds()
    ...
    --print("index ", 1, StringUtil.v2s(self.index))
    ...
    --print("index ", 2, StringUtil.v2s(self.index))
end
    ...
    for _, v in pairs(self.birds) do
        if v.status == 1 then
            ...
            if player ~= nil and v.collectValue > 0 then
                player:addFruitNum(v.collectValue)
                v.collectValue = 0
            end
        end
    end
...
function GameBird:carryBirdToWorld(id)
    for _, v in pairs(self.birds) do
        if tostring(v.id) == tostring(id) then
            ...
            local player = PlayerManager:getPlayerByUserId(self.userId)
            if player ~= nil then
                --local pos = self:selectPositionForBird(v)
                local index = 0
                for i, birdId in pairs(self.index) do
                    if tostring(birdId) == "0" then
                        self.index[i] = v.id
...
function GameBird:callBackBirdToBag(id)
    ...
    --print("index ", 3, StringUtil.v2s(self.index))
end
...
function GameBird:deleteBirdById(id)
    ...
    --print("index ", 4, StringUtil.v2s(self.index))
end
    ...
    for i, v in pairs(self.birds) do
        if tostring(v.id) == tostring(id) then
            ...
            if v.level ~= level then
                ...
                local player = PlayerManager:getPlayerByUserId(self.userId)
                if player ~= nil then
                    local num = self:getBirdsNumByLevel(v.level)
                    TaskHelper.dispatch(TaskType.HaveLevelBirdEvent, player.rakssid)
                    player:setBirdSimulatorBag()
                end
            end
...
function GameBird:attackMonster(id, entityId)
    ...
    for _, v in pairs(self.birds) do
        if tostring(id) == tostring(v.id) then
            local birdInfo = BirdConfig:getBirdInfoById(v.birdId)
            local rate = BirdConfig:getRateByIdAndLevel(v.birdId, v.level)
...
function GameBird:feed(birdId, foodId, num)
    ...
    local player = PlayerManager:getPlayerByUserId(self.userId)
    if player ~= nil then
        TaskHelper.dispatch(TaskType.FeedBirdEvent, player.rakssid, foodId, num)
    end
end
...
function GameBird:interruptConvert(id)
    ...
    for _, v in pairs(self.birds) do
        if tostring(id) == tostring(id) then
            player:addFruitNum(v.collectValue)
            v.convertState = ConvertState.STOP_CONVERT
            v.collectValue = 0
        end
