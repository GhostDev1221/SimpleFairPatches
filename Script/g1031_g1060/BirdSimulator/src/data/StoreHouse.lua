...
function StoreHouse:OnConsumptionResultBack(key, rakssid)
    ...
    TaskHelper.dispatch(TaskType.CollectPropEvent, rakssid, itemId, 1)
    self:OnChangeItemStatus(player, itemId, StoreItemStatus.OnUser)
    -- local birdStore = self:SyncStoreHouse(player)
    -- if birdStore ~= nil then
    --     HostApi.sendBirdSimulatorStoreItem(rakssid, birdStore)
    -- end
