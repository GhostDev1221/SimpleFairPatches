...
function GameMatch:savePlayerData(player, immediate)
    ...
    HostApi.setDBData(player.userId, DBDataListener.TAG_RANCHER_EXPLORE_INFO, json.encode(data), immediate or false)
end
...
function GameMatch:ifGameOverEnd()
    ...
    if seconds <= 0 then
        ...
        local players = PlayerManager:getPlayers()
        for k, v in pairs(players) do
            EngineUtil.sendGotoOtherGame(v.rakssid, v.userId, "g1031")
        end
...
function GameMatch:getDBDataRequire(player)
    HostApi.getDBDataByGameType("g1031", player.userId, DBDataListener.TAG_RANCHER_EXPLORE_INFO)
end
