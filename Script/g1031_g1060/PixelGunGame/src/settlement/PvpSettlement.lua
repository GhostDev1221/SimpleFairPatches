...
function PvpSettlement:doGameReward(players, winner)
    ...
    if blue.honorId ~= nil then
        blue_img = "set:season_rank.json image:ic_honor_lv" .. (blue.honorId + 1)
    end
    ...
    if red.honorId ~= nil then
        red_img = "set:season_rank.json image:ic_honor_lv" .. (red.honorId + 1)
    end
    ...
    for i, player in pairs(players) do
        ...
        HostApi.sendOpenPixelResult(player.rakssid, true, json.encode(own_data), 0)
