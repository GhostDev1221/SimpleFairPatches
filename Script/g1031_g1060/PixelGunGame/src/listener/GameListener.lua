...
function GameListener.onGameInit(GamePath, initPos)
    ...
    GameConfig:init()
    GameMatch:initMatch()
    HostApi.startDBServiceByGameType("g1042")
    initPos.x = GameConfig.initPos.x
    initPos.y = GameConfig.initPos.y
    initPos.z = GameConfig.initPos.z
    EngineWorld:stopWorldTime()
    BaseMain:setGameType("g1042")
    ReportManager:setGameType(GameMatch.gameType)
    HostApi.setServerGameType(GameMatch.gameType)
