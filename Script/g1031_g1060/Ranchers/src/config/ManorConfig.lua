...
function ManorConfig:initGate(config)
    for i, v in pairs(config.gate) do
        ...
        gate.initPos = VectorUtil.newVector3(tonumber(v.initPos[1]), tonumber(v.initPos[2]), tonumber(v.initPos[3]))
...
function ManorConfig:initFieldPos(config)
    for i, v in pairs(config) do
        local data = {}
        data.pos = VectorUtil.newVector3(tonumber(v.pos[1]), tonumber(v.pos[2]), tonumber(v.pos[3]))
...
function ManorConfig:prepareGate()
    for i, v in pairs(self.serviceCenter.gate) do
        EngineWorld:addActorNpc(v.initPos, v.yaw, "ranchers_door.actor", "", "", "", true, false)
    end

    for i, v in pairs(self.manor) do
        local pos = VectorUtil.newVector3(v.effectPosX, v.effectPosY, v.effectPosZ)
        EngineWorld:addActorNpc(pos, 0, "ranchers_door.actor", "", "", "", true, false)
    end
end
