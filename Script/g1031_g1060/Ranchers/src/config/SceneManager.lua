...
function SceneManager:initDataFromDB(userId, data, tag)
    ...
    if tag == DataBaseManager.TAG_RANCHER_EXPLORE_INFO then
        ...
        if #data ~= 0 then
            ...
            local src_data = json.decode(data)
            if self.userAchievementInfo[tostring(userId)]:initRanchExploreFromDB(src_data) then
                -- 保存探险数据，等后续对数据处理之后，再修改数据内容
                if src_data.money then
                    -- HostApi.log("src_data.money" .. tonumber(src_data.money))
                    local cover_data = {}
                    cover_data.money = tonumber(src_data.money)
