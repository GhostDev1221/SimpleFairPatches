...
function GameHouse:initDataFromDB(data)
    ...
    self:initHouseLevelFromDB(item.houseLevel)
    self:initHouseOffsetFromDB(item.offset)
end
...
function GameHouse:setHouseToWorld()
    ...
    if pos ~= nil then
        EngineWorld:getWorld():createOrDestroyHouseFromSchematic(houseInfo.fileName, pos, false, false, true)
    end
    ...
    if npcPos ~= nil then
        self.house.npcEntityId = EngineWorld:addSessionNpc(npcPos, houseInfo.npcYaw, SessionNpcConfig.MULTI_TIP_INTERACTION_NPC, npcInfo.name, npcInfo.actor, "body", "", json.encode(message))
    end
    self.isInWorld = true
end
...
function GameHouse:removeHouseFromWorld(houseLevel)
    ...
    local pos = ManorConfig:getVec3iByOffset(self.manorIndex, self.house.offset)
    if pos ~= nil then
        EngineWorld:getWorld():createOrDestroyHouseFromSchematic(houseInfo.fileName, pos, false, false, false)
