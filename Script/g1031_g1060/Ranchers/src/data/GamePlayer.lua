...
function GamePlayer:initDataFromDB(data)
    --HostApi.log(" GamePlayer:initDataFromDB : data = " .. data)
    local item = json.decode(data)
    self.level = item.level
    self.exp = item.exp
    self.giftNum = item.giftNum
    self.achievementPoint = item.achievementPoint
    self.prosperity = item.prosperity
    self.currentFieldNum = item.currentFieldNum
    self:fillInvetory(item.inventory)
    self:addExp(0)
    self:setCurrency(tonumber(item.money))
    self.fieldLevel = FieldLevelConfig:getFieldLevelByCurFieldNum(self.currentFieldNum)
    if item.isCanFlying ~= nil and item.isCanFlying == 1 then
...
function GamePlayer:savePlayerData(immediate)
    ...
    --HostApi.log("==========  GamePlayer:savePlayerData: player["..tostring(self.userId).."]    start  ==========")
    -- save manor area
    ...
    --HostApi.log("==========  GamePlayer:savePlayerData: player["..tostring(self.userId).."]    end ==========")
end
...
function GamePlayer:sendMsgToAllServer(msg, stayTime, rollTime)
    ...
    WebService:sendMsg(self.userId, 0, json_content, 3, 'game', "g1031")
end
