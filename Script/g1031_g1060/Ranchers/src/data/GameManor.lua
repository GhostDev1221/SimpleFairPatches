...
function GameManor:removeAllManorFromWorld()
    ...
    if pos ~= nil then
        ...
        pos.y = pos.y - 1
        pos.z = pos.z - 1
        EngineWorld:getWorld():createOrDestroyHouseFromSchematic(GameConfig.manorFileName, pos, false, false, true)
    end
end
...
function GameManor:initDataFromDB(data)
    --HostApi.log(" GameManor:initDataFromDB : data = " .. data)
    local item = json.decode(data)
    self.unlockedAreaCount = item.unlockedAreaCount
    self:initWaitForUnlockAreaFromDB(item.waitForUnlock)
    self:initUnlockedAreaFromDB(item.unlockedArea)
    self:initNameFromDB(item.name)
    self:updateCanUnlockArea()
end
...
function GameManor:addTemplateByAreaId(id)
    ...
    EngineWorld:getWorld():createOrDestroyHouseFromSchematic(area.fileName, pos, false, false, true)
end
...
function GameManor:removeTemplateByAreaId(id)
    ...
    local pos = VectorUtil.newVector3i(x, y, z)
    EngineWorld:getWorld():createOrDestroyHouseFromSchematic(area.fileName, pos, false, false, false)
end
