...
function PlayerListener.onPlayerReady(player)
    ...
    if manorInfo ~= nil then
        ...
        end, RanchersWeb:checkHasLand(player.userId))
        ...
        end, RanchersWeb:getMyClanProsperity(player.userId))
...
function PlayerListener.onDynamicAttrInitAndManorInfo(userId, targetUserId, manorIndex, class, team, region, pioneer, vip)
    HostApi.log("==== LuaLog: PlayerListener.onDynamicAttrInitAndManorInfo : " ..
    ...
    if tostring(targetUserId) ~= "0" and manorIndex > 0 then
        ...
        HostApi.log("==== LuaLog: [error] PlayerListener.onDynamicAttrInitAndManorInfo [targetUserId == 0] or [manorIndex <= 0] ")
    end
...
function PlayerListener.onPlayerCurrencyChange(rakssid, currency)
    ...
    if player ~= nil then
        ...
        if player.isReady == true then
            ...
            DataBaseManager:savePlayerData(player.userId, playerData, true)
        end
    end
end
...
function PlayerListener.onPlayerCallOnTargetManor(userId, targetId)
    ...
    if manorInfo == nil then
        RanchersWebResponse:registerCallBack(function(data, marked)
            ...
            if data ~= nil then
                EngineUtil.sendGotoOtherGame(callBackPlayer.rakssid, targetId, "g1031")
            else
                HostApi.sendCommonTip(callBackPlayer.rakssid, "ranch_tip_target_has_no_manor")
            end
        end, RanchersWeb:checkHasLand(targetId))
        return true
    end
...
function PlayerListener.onPlayerPutIntoRanchOrderCargo(rakssid, orderId, index)
    ...
    if taskInfo.askForHelpId ~= 0 then
        ...
        RanchersWebResponse:registerCallBack(function(result, marked)
            local callBackPlayer = PlayerManager:getPlayerByUserId(userId)
            if callBackPlayer == nil then
                return false
            end
        ...
        end, RanchersWeb:finishHelp(player.userId, player.userId, player.name, player:getSex(), uniqueId, taskInfo.askForHelpId))
    else
...
function PlayerListener.onPlayerRanchHelpFinish(rakssid, askForHelpId, type)
    ...
    local userId = player.userId
    RanchersWebResponse:registerCallBack(function(item, marked)
        if item == nil then
            return false
        end
        local callBackPlayer = PlayerManager:getPlayerByUserId(userId)
        if type == 1 then
            ...
            end, RanchersWeb:finishHelp(callBackPlayer.userId, callBackPlayer.userId, callBackPlayer.name, callBackPlayer:getSex(), uniqueId, askForHelpId))
        end
    ...
    end, RanchersWeb:helpListById(player.userId, askForHelpId))
...
function PlayerListener.onPlayerRanchReceiveMailReward(rakssid, mailId)
    ...
    local userId = player.userId
    WebResponse:registerCallBack(function(items, marked)
        if items == nil then
            return false
        end
        ...
        for i, v in pairs(items) do
            if tostring(v.id) == tostring(mailId) then
                if v.status == WebRequestType.EMAIL_STATUS_UNREAD then
                    ...
                    WebResponse:registerCallBack(function(result, marked)
                        if result == nil then
                            return false
                        end
                        local _callBackPlayer = PlayerManager:getPlayerByUserId(userId)
                    ...
                    end, WebService:updateMails(callBackPlayer.userId, v.id, WebRequestType.EMAIL_STATUS_RECEIVED))
                end
            end
        end
        
    end, WebService:checkMails(player.userId, "g1031"))
end
...
function PlayerListener.onPlayerRanchSendEveryDayGift(rakssid, toUserId)
    ...
    local userId = player.userId
    RanchersWebResponse:registerCallBack(function(result, marked, code)
        if code == 0 then
            return false
        end
        ...
        if code == 1 then
            ...
            end, RanchersWeb:checkHasLand(callBackPlayer.userId))
    ...
    end, RanchersWeb:giftSend(player.userId,fromUserTitle, fromContent, MailType.EveryDaySend, toUserId, toUserTitle,toContent,  MailType.EveryDayBeSend, "g1031"))
...
function PlayerListener.onPlayerRanchGoExplore(rakssid)
    ...
    EngineUtil.sendGotoOtherGame(rakssid, player.userId, "g1035")
...
function PlayerListener.onPlayerRanchSendInviteJoinGame(rakssid, targetUserId)
    ...
    RanchersWebResponse:registerCallBack(function (code, result)
        if code == 1 then
            HostApi.sendCommonTip(rakssid, "ranch_tip_send_invitation_success")
        else
            HostApi.sendCommonTip(rakssid, "ranch_tip_something_error")
        end
    end, RanchersWeb:sendInvitation(player.userId, targetUserId, player.name))
