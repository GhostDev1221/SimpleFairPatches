...
function player:check_ready()
    return self._data_ready and self.isReady
end
...
function player:on_death()
    ...
    HostApi.sendRespawnCountDown(self.rakssid, config.reviveTime)
end
...
function player:reward(rank, func)
    ...
    ReportManager:reportUserData(self.userId, self._score, rank, 1, 1)
end

------------------------------------------------------------------
-- db
function player:request_db_data()
    DBManager:getPlayerData(self.userId,
    define.PLAYER_DB_CHUNK_DEFAULT)
end

function player:receive_db_data(data)
    if #data > 0 then
        local ret, json_data = pcall(json.decode, data)
        if not ret then
            utils.log("player data error", json_data)
            return false
        end

        self._score_record = json_data.score_record or {}
        for _, item in ipairs(json_data.items or {}) do
            self:addItem(item[1], item[2], 0)
        end
    end
...
function player:save_db_data(immediate)
    ...
    DBManager:savePlayerData(self.userId,
        define.PLAYER_DB_CHUNK_DEFAULT,
        json.encode(data),
        immediate)
end
