...
function GamePlayer:setSoldierAttribute(soldier)
    if soldier then
        ...
        for i, v in pairs(soldier.inventory) do
            if v.fm.id ~= 0 then
                self:addEchantmentItem(v.id, v.num, 0, v.fm.id, v.fm.lv)
            else
...
function GamePlayer:skillReleaseToxic(range, duration, damage, poisonOnTime)
    ...
    GameMatch:poisonRangeAttack(self.rakssid, pos, range, duration, damage, poisonOnTime)
end
...
function GamePlayer:buildShowName()
    ...
    -- pureName line
    local disName = TextFormat.colorBlue .. self.name
    if self.staticAttr.lv > 0 then
        disName = TextFormat.colorGold .. "[Lv" .. tostring(self.staticAttr.lv) .. "]" .. TextFormat.colorBlue .. self.name
    end
    ...
    if self.isHunter == true then
        nameList[nameListNum] = self:getHunterName()
        nameListNum = nameListNum + 1
    end
