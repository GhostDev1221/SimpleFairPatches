...
function ManorNpcConfig:initSessionNpc(config)
    for i, v in pairs(config) do
        local sessionNpc = {}
        sessionNpc.pos = VectorUtil.newVector3(tonumber(v.initPos[1]), tonumber(v.initPos[2]), tonumber(v.initPos[3]))
...
function ManorNpcConfig:initRankNpc(config)
    for i, v in pairs(config) do
        local rankNpc = {}
        rankNpc.pos = VectorUtil.newVector3(tonumber(v.initPos[1]), tonumber(v.initPos[2]), tonumber(v.initPos[3]))
...
function ManorNpcConfig:initManorNpc(config)
    for i, v in pairs(config) do
        local npc = {}
        npc.pos = VectorUtil.newVector3(tonumber(v.initPos[1]), tonumber(v.initPos[2]), tonumber(v.initPos[3]))
...
function ManorNpcConfig:initManor(config)
    for i, v in pairs(config) do
        ...
        manor.telePosition = VectorUtil.newVector3(tonumber(v.telePosition[1]), tonumber(v.telePosition[2]), tonumber(v.telePosition[3]))
        manor.signPosition = VectorUtil.newVector3i(tonumber(v.signPosition[1]), tonumber(v.signPosition[2]), tonumber(v.signPosition[3]))
...
function ManorNpcConfig:prepareManorNpc()
    for i, v in pairs(self.manorNpc) do
        if v.type == 1 then
            v.entityId = EngineWorld:addSessionNpc(v.pos, v.yaw, ManorNpcConfig.SessionType_GET_MANOR, v.name, v.actor, "body", "",v.content)
        else
            v.entityId = EngineWorld:addSessionNpc(v.pos, v.yaw, ManorNpcConfig.SessionType_SELL_MANOR, v.name, v.actor, "body", "",v.content)
        end
    end
end
...
function ManorNpcConfig:prepareSessionNpc()
    for i, v in pairs(self.sessionNpc) do
        v.entityId = EngineWorld:addSessionNpc(v.pos, v.yaw, ManorNpcConfig.SessionType_TIP_NPC, v.name, v.actor, "body", "",v.message)
    end
end
...
function ManorNpcConfig:newManorInfo(player, level)
    ...
    if yardInfo ~= nil then
        ...
        if player.dynamicAttr.pioneer then
            info.price = yardInfo.blockymodsPrice
        else
            info.price = yardInfo.blockmanPrice
        end
        ...
        if player.dynamicAttr.pioneer then
            ...
            nextInfo.price = yardInfo.nextBlockmanPrice
        end
        nextInfo.value = 0
        nextInfo.houseId = 0
