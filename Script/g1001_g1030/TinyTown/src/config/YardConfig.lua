...
function YardConfig:buildFence(manorIndex, yardLevel)
    ...
    if yard ~= nil then
        ...
        EngineWorld:getWorld():createOrDestroyHouseFromSchematic(fileName, vec3, false, false, true)
    else
...
function YardConfig:destroyFence(manorIndex, yardLevel)
    ...
    if yard ~= nil then
        local fileName = yard.fenceName
        EngineWorld:getWorld():createOrDestroyHouseFromSchematic(fileName, vec3, false, false, false)
    else
