...
function FurnitureConfig:initFurnitureShow(config)
    for i, v in pairs(config) do
        ...
        furnitureShow.position = VectorUtil.newVector3(tonumber(v.position[1]), tonumber(v.position[2]), tonumber(v.position[3]))
...
function FurnitureConfig:prepareFurnitureShow()
    for i, v in pairs(self.furnitureShow) do
        EngineWorld:addSessionNpc(v.position, v.yaw, ManorNpcConfig.SessionType_MANOR_FURNITURE, " ", v.actor, "body", v.bodyId)
    end
end
...
function FurnitureConfig:newFurniture(player)
    ...
    for i, v in pairs(self.furniture) do
        ...
        if player.dynamicAttr.pioneer then
            ...
            furniture.price = v.blockmanPrice
        end
        furniture.id = v.id
...
function FurnitureConfig:placeFurnitureSure(player, furnitureId, furniture)
    ...
    local entityId = EngineWorld:addSessionNpc(vec3, yaw, ManorNpcConfig.SessionType_MANOR_FURNITURE, " ", furniture.name, "body", furniture.bodyId, tostring(furnitureId))
