...
function BlockListener.onBlockPlace(rakssid, blockId, blockMeta, vec3)
    ...
    if blockId >= 900 then
        vec3.y = vec3.y - 1 -- 种子放置的坐标 -1 后与底下泥土的坐标相等
        if YardConfig:isInGarden(player.manorIndex, player.yardLevel, vec3) then
    ...
    if YardConfig:isInYard(player.manorIndex, player.yardLevel, vec3) then
        for i, v in pairs(BlockConfig.block) do
            if v.id == blockId and ((v.meta == blockMeta) or (blockId == 50 or blockId == 65))  then
                local temporaryStore = {}
                local offsetVec3 = YardConfig:getOffsetByVec3i(player.manorIndex, vec3)
                temporaryStore.id = tonumber(blockId)
                temporaryStore.meta = tonumber(blockMeta)
                temporaryStore.offsetVec3 = offsetVec3
                GameMatch:initTemporaryStore(player.userId, temporaryStore)
                player.modify_block = true
                return true
