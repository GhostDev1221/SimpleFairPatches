...
function PlayerListener.onDynamicAttrInitAndManorInfo(platformId, targetUserId, manorIndex, class, team, region, pioneer, vip)
    HostApi.log("==== LuaLog: PlayerListener.onDynamicAttrInitAndManorInfo  --- start --- : platformId = " .. tostring(platformId) .. ", targetUserId = " .. tostring(targetUserId) .. ", manorIndex = " .. manorIndex)
    if tostring(targetUserId) ~= "0" and manorIndex > 0 then
        ...
        if GameMatch.userManorIndex[tostring(targetUserId)] == nil then
            ...
            HostApi.log("==== LuaLog: PlayerListener.onDynamicAttrInitAndManorInfo :  use memory data ...  GameMatch.userManorIndex[".. tostring(targetUserId) .. "] = " .. GameMatch.userManorIndex[tostring(targetUserId)])
            ...
            if userInfo ~= nil then
                ...
                HostApi.log("==== LuaLog: PlayerListener.onDynamicAttrInitAndManorInfo : memory data is nil,  program has error  !!!!! ")
            end
        end
    else
        HostApi.log("==== LuaLog: PlayerListener.onDynamicAttrInitAndManorInfo [targetUserId == 0] or [manorIndex <= 0] ")
    end
end
...
function PlayerListener.onPlayerBuildHouse(rakssid, houseId)
    ...
    for i, v in pairs(GameMatch.userHouseId[tostring(player.userId)]) do
        if v ~= nil then
            v.state = 0
        end
    end
                    ...
                    for i, v in pairs(furnitures) do
                        if v ~= nil then
                            ...
                            if furniture ~= nil then
                                ...
                                data.actorName = furniture.name
                                if v.state == 1 then
                                    local entityId = EngineWorld:addSessionNpc(vec3, v.offset, ManorNpcConfig.SessionType_MANOR_FURNITURE, " ", furniture.name, "body", furniture.bodyId, tostring(furniture.id))
