...
function GamePlayer:showRespawn()
    HostApi.sendRespawnCountDown(self.rakssid, GameConfig.respawnTime)
end
...
function GamePlayer:onDie()
    if self.isLife then
        ...
        self.team:subPlayer()
        RespawnManager:removeSchedule(self.rakssid)
...
function GamePlayer:addEnchantmentGoodsItem(enchantmentItem)
    ...
    self:addEchantmentsItem(enchantmentItem.itemId, enchantmentItem.num, enchantmentItem.meta, enchantments)
end

function GamePlayer:initDataFromDB(data)
    if #data ~= 0 then
        local json_data = json.decode(data)
        if json_data.money ~= nil then
            self:setCurrency(json_data.money)
        end
        if json_data.enchantmentItem ~= nil then
            for i, v in pairs(json_data.enchantmentItem) do
...
function GamePlayer:savePlayerData(immediate)
    ...
    DBManager:savePlayerData(self.userId, 1, json.encode(data), immediate)
end
