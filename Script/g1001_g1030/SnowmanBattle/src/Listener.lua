...
function Listener.onGameInit(GamePath, initPos)
    ...
    Shop:init()
    initPos.x = GameConfig.teams[1].waitPos[1]
    initPos.y = GameConfig.teams[1].waitPos[2]
    initPos.z = GameConfig.teams[1].waitPos[3]
...
function Listener.onBlockPlace(rakssid, blockId, blockMeta, vec3)
    return true
end
...
function Listener.onPlayerRespawn(player)
    player.is_death = false
    player:addNightVisionPotionEffect(1800)
