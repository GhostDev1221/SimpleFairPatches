...
function Player:buildShowName()
    ...
    if self.staticAttr.lv > 0 then
        disName = TextFormat.colorGold .. "[Lv" .. tostring(self.staticAttr.lv) .. "]"
    end

    disName = disName .. self:getDisplayName()
...
function Player:on_death()
    if self.respawnCount > 0 then
        ...
        HostApi.sendRespawnCountDown(self.rakssid, GameConfig.respawn.wait)
