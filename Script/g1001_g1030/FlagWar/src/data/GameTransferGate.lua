...
function GameTransferGate:ctor(config)
    ...
    self.records = {}
    self.enemys = {}
    self.destination = VectorUtil.newVector3(tonumber(config.destination[1]), tonumber(config.destination[2]), tonumber(config.destination[3]))
end
...
function GameTransferGate:onTick(tick)
    for i, record in pairs(self.records) do
        ...
        if second >= self.time then
            ...
            MsgSender.sendBottomTipsToTarget(record.rakssid, 2, Messages:transferGateHint(self.time - second))
        end
    end
end

function GameTransferGate:telePlayerToDestination(record)
    local player = PlayerManager:getPlayerByRakssid(record.rakssid)
    if player ~= nil then
