...
function GameFlag:ctor(config)
    ...
    self.initPos = VectorUtil.newVector3(tonumber(config.initPos[1]), tonumber(config.initPos[2]), tonumber(config.initPos[3]))
...
function GameFlag:onAddFlag()
    self.entityId = EngineWorld:addActorNpc(self.initPos, self.yaw, self.actor, "", "idle")
end
