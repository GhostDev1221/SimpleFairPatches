...
function GameFlagStation:ctor(config)
    ...
    self.flagPos = VectorUtil.newVector3(tonumber(config.flagPos[1]), tonumber(config.flagPos[2]), tonumber(config.flagPos[3]))
    self.flagYaw = tonumber(config.flagPos[4])
    self.area = Area.new(config.area)
    self.haloEffect = config.haloEffect
    self.buff = {}
...
function GameFlagStation:onPlayerEnter(player)
    ...
    if self.hasFlag == false then
        ...
        self.hasFlag = true
        self.entityId = EngineWorld:addActorNpc(self.flagPos, self.flagYaw, self.actor, "", "idle", self.haloEffect)
