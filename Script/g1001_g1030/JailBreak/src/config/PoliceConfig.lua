...
function PoliceConfig:initInitPos(config)
    for i, pos in pairs(config) do
        self.initPos[i] = VectorUtil.newVector3(tonumber(pos.x), tonumber(pos.y), tonumber(pos.z))
    end
end

function PoliceConfig:initConfineRoomPos(config)
    for i, pos in pairs(config) do
        self.confineRoomPos[i] = VectorUtil.newVector3(tonumber(pos.x), tonumber(pos.y), tonumber(pos.z))
    end
end
