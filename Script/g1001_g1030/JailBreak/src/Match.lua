...
function GameMatch:saveAllPlayerData()
    if self.curTick == 0 or self.curTick % 10 ~= 0 then
        return
    end
    local players = PlayerManager:getPlayers()
...
function GameMatch:savePlayerData(player, immediate)
    ...
    if player.role == GamePlayer.ROLE_CRIMINAL or player.role == GamePlayer.ROLE_PRISONER then
        DBManager:savePlayerData(player.userId, GamePlayer.ROLE_CRIMINAL, json.encode(data), immediate)
    else
        DBManager:savePlayerData(player.userId, GamePlayer.ROLE_POLICE, json.encode(data), immediate)
    end
end
...
function GameMatch:policeRoleDataCache(player, role, policeData)
    ...
    if #policeData == 0 then
        data.police.lv = policeConfig.lv
        data.police.money = 0
        data.police.merit = 0
        data.police.name = policeConfig.nameKey
    else
        local oldData = json.decode(policeData)
        local config = PoliceConfig:getDataByMerit(oldData.merit)
        data.police.lv = config.lv
        data.police.money = oldData.money
        data.police.merit = oldData.merit
        data.police.name = config.nameKey
    end
...
function GameMatch:criminalRoleDataCache(player, role, criminalData)
    ...
    if #criminalData == 0 then
        data.criminal.lv = criminalConfig.lv
        data.criminal.money = 0
        data.criminal.threat = 0
        data.criminal.name = criminalConfig.nameKey
    else
        local oldData = json.decode(criminalData)
        local config = CriminalConfig:getDataByThreat(oldData.threat)
        data.criminal.lv = config.lv
        data.criminal.money = oldData.money
        data.criminal.threat = oldData.threat
        data.criminal.name = config.nameKey
    end
