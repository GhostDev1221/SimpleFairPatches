...
function GameMatch:telePlayerToGame()
    local players = PlayerManager:getPlayers()
    for _, player in pairs(players) do
...
function GameMatch:sendRealRankData(player)
    local playselist = self:SortPlayersRank()
    local datas = {}
    datas.ranks = {}
    for index = 1, #playselist do
        local item = {}
        local _player = playselist[index]
    ...
    local rank = 0
    for i, v in pairs(playselist) do
        rank = rank + 1
