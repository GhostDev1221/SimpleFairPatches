...
function DomainConfig:initDomains(domains)
    for id, c_item in pairs(domains) do
        ...
        domain.startPos = VectorUtil.newVector3i(tonumber(c_item.startX), tonumber(c_item.startY), tonumber(c_item.startZ))
        domain.respwanPos = VectorUtil.newVector3(tonumber(c_item.respwanX), tonumber(c_item.respwanY), tonumber(c_item.respwanZ))
