...
function EquipBoxConfig:initEquipboxs(equipboxs)
    for id, c_item in pairs(equipboxs) do
        ...
        item.position = VectorUtil.newVector3(tonumber(c_item.x), tonumber(c_item.y), tonumber(c_item.z))
        ...
        if #effect > 4 then
            item.effect.pos = VectorUtil.newVector3(tonumber(effect[4]), tonumber(effect[5]), tonumber(effect[6]))
        end
