...
function BuildingConfig:initBuildings(buildings)
    for id, c_item in pairs(buildings) do
        ...
        local item = {}
        item.id = id
        item.startPos = VectorUtil.newVector3i(tonumber(c_item.startX), tonumber(c_item.startY), tonumber(c_item.startZ))
        ...
        item.npcPos = VectorUtil.newVector3(tonumber(c_item.npcX), tonumber(c_item.npcY), tonumber(c_item.npcZ))
