...
function GameGate:openGate()
    self.isCanClose = false
    self.isOpen = true
    EngineWorld:getWorld():createOrDestroyHouseFromSchematic(self.config.fileName, self.pos, self.xImage, self.zImage, false)
end

function GameGate:closeGate()
    self.isOpen = false
    EngineWorld:getWorld():createOrDestroyHouseFromSchematic(self.config.fileName, self.pos, self.xImage, self.zImage, true)
end
