...
function GamePlayer:initDataFromDB(data)
    if #data ~= 0 then
        ...
        --初始化超级英雄氪金职业id列表
        for _, occId in pairs(json_data.supOccIds) do
            local isAdd = true
        ...
        --初始化玩家剩余货币信息
        for _, item in pairs(json_data.walletInfo) do
            if item.coinNum > 0 then
                table.insert(self.walletInfo, item)
            end
        end
        --初始化玩家购买过的职业升级装备柜
        self.upgradeEquips = json_data.upgradeEquips or {}
    end
...
function GamePlayer:buildShowName()
    ...
    if self.config and self.config.displayName then
        nameList[nameListNum] = self.config.displayName
        nameListNum = nameListNum + 1
    end

    nameList[nameListNum] = TextFormat.colorWrite .. self.name
    nameListNum = nameListNum + 1
...
function GamePlayer:savePlayerData()
    ...
    DBManager:savePlayerData(self.userId, 1, json.encode(data), true)
end
...
function GamePlayer:unlockOccupation(extra)
    ...
    if extras[1] == "Single" then
        ...
        HostApi.log("occupationInfo.singleCurrencyType = " .. occupationInfo.singleCurrencyType)
        HostApi.log("occupationInfo.singlePrice = " .. occupationInfo.singlePrice)
        self:payUnlockOccupation(moneyType, money, extra, npc)
        return
    end
    ...
    if extras[1] == "Perpetual" then
        ...
        HostApi.log("occupationInfo.perpetualCurrencyType = " .. occupationInfo.perpetualCurrencyType)
        HostApi.log("occupationInfo.perpetualPrice = " .. occupationInfo.perpetualPrice)
        self:payUnlockOccupation(moneyType, money, extra, npc)
        return
    end
...
function GamePlayer:payUnlockOccupation(moneyType, money, extra, npc)
    local extras = StringUtil.split(extra, "=")
    if #extras ~= 2 then
        return
    end
    ...
    if tonumber(moneyType) == 1 then
        self:consumeDiamonds(moneyType, money, extra, false)
