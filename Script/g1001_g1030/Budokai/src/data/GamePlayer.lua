...
function GamePlayer:initDataFromDB(data)
    if #data ~= 0 then
        ...
        self.money = json_data.money or 0
        self.games = json_data.games or 0
        self.wingames = json_data.wingames or 0
        self.integral = json_data.integral or 0
        self.pkscore = json_data.pkscore or 0
        self.titles = json_data.titles or {}
        self.talents = json_data.talents or {}
    end
...
function GamePlayer:buildShowName()
    ...
    for id, status in pairs(self.titles) do
        if status == 3 then
            ...
            if title ~= nil then
                nameList[nameListNum] = title.color .. title.title
                nameListNum = nameListNum + 1
                break
            end
        end
    ...
    nameList[nameListNum] = self:getSegmentName()
    nameListNum = nameListNum + 1
    ...
    nameList[nameListNum] = TextFormat.colorWrite .. self.name .. status
    nameListNum = nameListNum + 1
