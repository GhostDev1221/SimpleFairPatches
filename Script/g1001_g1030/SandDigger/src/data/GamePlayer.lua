...
function GamePlayer:getDataFromDB(data)
    if #data ~= 0 then
        ...
        self:setCurrency(tonumber(json_data.money))
        self.score = tonumber(json_data.score)
        self.backpackLevel = tonumber(json_data.backpackLevel)
        self:fillInvetory(json_data.inventory)
