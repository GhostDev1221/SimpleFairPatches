...
function GamePlayer:onKill()
    ...
    elseif self.multiKill >= 3 then
        self.score = self.score + ScoreConfig.KILL_3_SCORE
        MsgSender.sendMsg(IMessages:msgThirdKill(self:getDisplayName()))
...
function GamePlayer:upgradeStore()
    if self.team ~= nil then
        MsgSender.sendMsg(Messages:msgUpgrade(self:getDisplayName(), Messages:msgStore()))
        self.team:upgradeStore()
    end
end

function GamePlayer:upgradeIron()
    if self.team ~= nil then
        MsgSender.sendMsg(Messages:msgUpgrade(self:getDisplayName(), Messages:msgIronMoney()))
        self.team:upgradeIron()
    end
end

function GamePlayer:upgradeGold()
    if self.team ~= nil then
        MsgSender.sendMsg(Messages:msgUpgrade(self:getDisplayName(), Messages:msgGoldMoney()))
        self.team:upgradeGold()
    end
end
...
function GamePlayer:sendOpenChantment(type)
    ...
    result.consumeBg = "set:enchantment.json image:consume_bedwar_bg"
        ...
        if item.id > 0 and item.id < 10000 then
            if MerchantConfig.equipment[item.id] and MerchantConfig.equipment[item.id] == 1 then
                if item.enchantId and item.enchantLv then
                    self:addEchantmentItem(item.id, item.num, 0, item.enchantId, item.enchantLv)
                else
