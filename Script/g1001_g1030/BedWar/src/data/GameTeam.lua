...
function GameTeam:upgradeStore()
    self.merchant:upgrade()
end

function GameTeam:upgradeIron()
    if self.ironLv < GameConfig.maxLevel then
        self.ironLv = self.ironLv + 1
    end
end

function GameTeam:upgradeGold()
    if self.goldLv < GameConfig.maxLevel then
        self.goldLv = self.goldLv + 1
    end
end
