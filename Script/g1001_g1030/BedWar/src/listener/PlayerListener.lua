...
function PlayerListener.onPlayerKeepItem(rakssid, isKeepItem)
    ...
    player.isKeepItem = isKeepItem
    if isKeepItem then
        local Config = KeepItemConfig:getKeepItemByTimes(player.keepItemTimes)
        player:consumeDiamonds(1, Config.price, "keepItem", true)
    end

    if player.keepItemSource == 0 then
        HostApi.sendRespawnCountDown(player.rakssid, 3)
    end
end
...
function PlayerListener.onPlayerReady(player)
    ...
    for i, v in pairs(GameMatch.Merchants) do
        v:syncPlayer(player.rakssid)
    end
...
function PlayerListener.onPlayerBuyCommodity(rakssid, type, goodsInfo, attr, msg)
    ...
    if goodsId == GameConfig.storeUpgradeId then
        if player.team.storeLv < GameConfig.maxLevel then
            player:upgradeStore()
    ...
    if goodsId == GameConfig.ironUpgradeId then
        if player.team.ironLv < GameConfig.maxLevel then
            player:upgradeIron()
            attr.isAddGoods = false
            return true
        else
    ...
    if goodsId == GameConfig.goldUpgradeId then
        if player.team.goldLv < GameConfig.maxLevel then
            player:upgradeGold()
            attr.isAddGoods = false
            return true
        else
    ...
    local money = MoneyConfig:getPublicMoneyByUpgradeId(goodsId)
    if money == nil then
        return false
    end
    if money.level < GameConfig.maxLevel then
        MsgSender.sendMsg(Messages:msgUpgrade(player:getDisplayName(), Messages:msgPublicMoney()))
...
function PlayerListener.onPlayerUseCommonItem(rakssId, itemId)
    ...
    if itemId >= 462 and itemId <= 466 then
        -- tp
...
function PlayerListener.onPlayerEnchantmentQuick(rakssid, equipId, index)
    ...
    player:consumeDiamonds(1, price, "enchantmentQuick" .. "#" .. equipId .. "#" .. effectId .. "#" .. effectLevel, true)
end
