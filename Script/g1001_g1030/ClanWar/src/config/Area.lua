...
function Area:setRandomInv(randomInv)
    for i, v in pairs(randomInv) do
        self.randomInv[i] = {}
        self.randomInv[i].beginPos = VectorUtil.newVector3i(tonumber(v.beginPos[1]), tonumber(v.beginPos[2]), tonumber(v.beginPos[3]))
        self.randomInv[i].endPos = VectorUtil.newVector3i(tonumber(v.endPos[1]), tonumber(v.endPos[2]), tonumber(v.endPos[3]))
