...
function BlockListener.onBlockPlace(rakssid, blockId, blockMeta, vec3)
    ...
    if player ~= nil then
        local canPlace1, isPlaceFlag1 = GameMatch.Teams[1]:onBlockPlace(blockId, blockMeta, vec3)
        local canPlace2, isPlaceFlag2 = GameMatch.Teams[2]:onBlockPlace(blockId, blockMeta, vec3)
