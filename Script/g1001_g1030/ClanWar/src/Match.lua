...
function GameMatch:doReward()
    ...
    self:addRecords(win)
    local resJson = ClanWarWeb:GetReward(self:getRewardRequestBody())
    if resJson ~= nil and string.len(resJson) ~= 0 and resJson ~= "{}" then
        local response = json.decode(resJson)
        if response.code == 1 and response.data ~= nil then
            self:sendReward(response.data)
        else
            HostApi.log("do reward error : response = " .. resJson)
        end
    end
end
...
function GameMatch:addRecords(win)
    ...
    local players = PlayerManager:getPlayers()
    for i, v in pairs(players) do
        if v.dynamicAttr.pioneer == false then
...
function GameMatch:addRecord(player, win)
    ...
    self.record[self.recordNum].region = player.dynamicAttr.region
