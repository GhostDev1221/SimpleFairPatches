...
local function select(list, pred)
    ...
    for i = 1, #list do
        ...
        if v and pred(v, i) then
            while string.sub(v, #v, #v + 1) == ' ' do
                v = string.sub(v, 1, -2)
            end
            tinsert(selected, { data = v, line = i })
        end
    end
    return selected
end
    ...
    while true do
        ...
        elseif c == ',' then
            ...
            if value.t == ':' or value.t == '{' or value.t == '[' then
                error("Error" .. " [lines:" .. lines[1].line .. "]")
            end
        ...
        elseif c == ']' then
            ...
            if not s then
                error('invalid flowstyle line: ' .. line .. " [lines:" .. lines[1].line .. "]")
            end
...
local function parseblockstylestring(line, lines, indent)
    if #lines == 0 then
        error("failed to find multi-line scalar content " .. " [lines:" .. lines[1].line .. "]")
    end
    local s = {}
    local firstindent = -1
    local endline = -1
    ...
    elseif line == '>-' then
        ...
        error('invalid blockstyle string:' .. line .. " [lines:" .. lines[1].line .. "]")
    end
    local eonl = 0
    for i = #s, 1, -1 do
        if s[i] == '' then
...
local function parsescalar(line, lines, indent)
    ...
    if sfind('\'"!$', ssub(line, 1, 1), 1, true) then
        error('unsupported line: ' .. line .. " [lines:" .. lines[1].line .. "]")
    end
...
local function parseseq(line, lines, indent)
    ...
    while #lines > 0 do
        ...
        elseif level > indent then
            error("found bad indenting in line: " .. line .. " [lines:" .. lines[1].line .. "]")
        end
        ...
        if sfind(rest, '^[^\'\"%s]*:') then
            -- Inline nested hash
            local indent2 = j
            lines[1].data = string.rep(' ', indent2) .. rest
        ...
        elseif isemptyline(rest) then
            ...
            if sfind(lines[1].data, '^%s*%-') then
                local nextline = lines[1].data
...
local function parseset(line, lines, indent)
    if not isemptyline(line) then
        error('not seq line: ' .. line .. " [lines:" .. lines[1].line .. "]")
    end
    ...
    while #lines > 0 do
        ...
        elseif sfind(rest, '^%s+$') then
            ...
            if sfind(lines[1].data, '^%s*%?') then
                local indent2 = countindent(lines[1].data)
                if indent2 == indent then
                    -- Null array entry
                    set[null] = true
                else
        ...
        elseif rest then
            ...
            error("failed to classify line: " .. line .. " [lines:" .. lines[1].line .. "]")
        end
    end
    return set
end
...
function parsemap(line, lines, indent)
    if not isemptyline(line) then
        error('not map line: ' .. line .. " [lines:" .. lines[1].line .. "]")
    end
    ...
    while #lines > 0 do
        ...
        if not isemptyline(line) then
            ...
            elseif sfind(lines[1].data, '^%s*%?') then
                local indent2 = countindent(lines[1].data)
...
local function parsedocuments(lines)
    ...
    if sfind(lines[1].data, '^%%YAML') then
        tremove(lines, 1)
    end
    ...
    while #lines > 0 do
        ...
        elseif #lines == 0 or startswith(line, '---') then
            -- A naked document
            tinsert(root, null)
            while #lines > 0 and not sfind(lines[1].data, '---') do
                tremove(lines, 1)
            end
            in_document = false
        ...
        elseif not in_document and #root > 0 then
            ...
            error('parse error: ' .. line .. " [lines:" .. lines[1].line .. "]")
